...
from intertidal.io import _write_thumbnail
...
def _write_thumbnail_cem(da: xr.DataArray, path: str, max_resolution: int = 320):
    ...
    jpeg_data = (
        reprojected
        .odc.colorize(vmin=0, vmax=255, cmap=cmap)
        .odc.compress("jpeg", 85, transparent=[255, 255, 255])  # White as transparent
    )
    ...
    def __post_init__(self):
        ...
        if self.naming_conventions == 'dea_c3':
            ...
            if 'continental' in self.study_area.lower() or self.study_area == 'AU':
                ...
                self.title = f'{self.product_name}_{self.study_area}_{self.year}--{self.freq}'
            ...
            if len(version_parts) >= 3:
                ...
                version_suffix = self.version  # Fallback to full version if format unexpected
        ...
        explorer_domain = 'explorer.dev.dea.ga.gov.au' if self.dev else 'explorer.dea.ga.gov.au'
...
def get_platform_instrument_s2ls(year: int) -> Tuple[str, str]:
    """
    ...
    Returns:
        Tuple of (platform string, instrument string)
    """
    if year <= 2020:
...
def get_platform_instrument_s2(year: int) -> Tuple[str, str]:
    """
...
def load_lineage(lineage_file: pathlib.Path, verbose: bool = False) -> Dict[str, List[str]]:
    """
...
def extract_lineage_from_tiles(
    ...
    tile_naming = tile_naming_conventions if tile_naming_conventions else naming_conventions
    
    # Build pattern based on TILE naming convention
    if tile_naming == 'dea_c3':
        # dea_c3: includes version in path
        pattern = f'{s3_bucket}/{product}/{version}/x*/y*/{year}--{freq}/*.stac-item.json'
    else:
    ...
    compiled_lineage_dict = {key: sorted(list(values)) for key, values in compiled_lineage.items()}
...
def generate_metadata(
    ...
    if config.product_family == "intertidal":
        platform, instrument = get_platform_instrument_s2ls(year_int)
    elif config.product_family in ("tidal_composites", "coastalecosystems"):
        platform, instrument = get_platform_instrument_s2(year_int)
    else:
    ...
    with DatasetPrepare(
        collection_location="s3://dea-public-data-dev" if config.dev else "s3://dea-public-data",
...
@click.option('--product', required=True, help='Product name (e.g., ga_s2_coastalecosystems_cyear_3_v1)')
@click.option('--version', required=True, help='Product version (e.g., 1-0-0)')
@click.option('--product-family', default=None, help='Product family (auto-detected from product name if not specified: intertidal, tidal_composites, or coastalecosystems)')
...
@click.option('--product-maturity', default='stable', help='Product maturity (default: stable)')
@click.option('--dataset-maturity', default='final', help='Dataset maturity (default: final)')
@click.option('--dataset-version', default='1.0.0', help='Dataset version (default: 1.0.0)')
@click.option('--nodata-value', default=255, type=int, help='NoData value (default: 255)')
@click.option('--naming-conventions', default='dea_c3', type=click.Choice(['dea', 'dea_c3']), 
              help='Naming convention: dea (includes maturity in filename) or dea_c3 (no maturity, for continental mosaics, default: dea_c3)')
@click.option('--dev/--prod', default=True, help='Use dev or production S3 bucket (default: dev)')
@click.option('--lineage-file', type=click.Path(exists=True, path_type=pathlib.Path),
              help='Path to compiled lineage JSON file')
@click.option('--auto-lineage', is_flag=True, help='Auto-generate empty lineage (for testing/development)')
@click.option('--extract-lineage-from-tiles/--no-extract-lineage-from-tiles', 'extract_from_tiles', default=True, 
              help='Extract lineage from tile STAC files in S3 (default: True)')
@click.option('--tile-dir', default='dea-public-data-dev/derivative',
              help='S3 bucket/path for tiles (default: dea-public-data-dev/derivative)')
@click.option('--output-dir', type=str, default='metadata',
              help='Output destination (S3 or local path, default: metadata)')
@click.option('--validate/--no-validate', default=True, help='Validate generated metadata (default: True)')
...
def generate(
    ...
    if product_family is None:
        if 'intertidal' in product.lower():
            ...
            click.echo(f"Error: Could not auto-detect product family from product name: {product}", err=True)
            click.echo("Product name must contain 'intertidal', 'tidal_composites', or 'coastalecosystems'", err=True)
    ...
    if auto_lineage:
        ...
        try:
            ...
            tile_naming = 'dea_c3' if product_family in coastal_products else naming_conventions
        ...
        click.echo("\nOptions:", err=True)
        click.echo("  1. Use --extract-lineage-from-tiles (default, already enabled)", err=True)
...
@click.option('--dev/--prod', default=True, help='Use dev or production bucket (default: dev)')
def show_paths(
    year: str,
    product: str,
    version: str,
