...
def pixel_tides_ensemble(
    ...
    **pixel_tides_kwargs,
    ...
    """
    Generate an ensemble tide model, choosing the best three
    tide models for any coastal location using ancillary point
    data (e.g. altimetry observations or NDWI correlations
    along the coastline).

    This function generates an ensemble of tidal height
    predictions for each pixel in a satellite dataset. Firstly,
    tides from multiple tide models are modelled into a low
    resolution grid using `pixel_tides`. Ancillary points are
    then loaded and interpolated to the same grid to serve as
    weightings. These weightings are used to retain only the
    top three tidal models, and remaining top models are
    combined into a single ensemble output for each time/x/y.
    The resulting ensemble tides are then resampled and
    reprojected to match the high-resolution satellite data.
    ...
    ancillary_points : str
        Path to a file containing point correlations for
        different tidal models.
    top_n : integer, optional
        The number of top models to use in the ensemble
        calculation. Default is 3, which will calculate a
        median of the top 3 models.
    models : list or None, optional
        An optional list of tide models to use for the
        ensemble model. Default is None, which will use
        "FES2014", "FES2012", "EOT20", "TPXO8-atlas-v1",
        "TPXO9-atlas-v5", "HAMTIDE11", "GOT4.10".
    interp_method : str, optional
        Interpolation method used to interpolate correlations
        onto the low-resolution tide grid. Default is "nearest".
    reduce_method : str, optional
        The statistic used to combine results from the identified `top_n`
        models. Default is "median"; also supports "mean".
    ...
    weights_ds : xarray.Dataset
        Dataset containing weights for each tidal model used in the
        ensemble.
    """
    ...
    ranks_gdf = (
        gpd.read_file(ancillary_points)[[f"rank_{m}" for m in models] + ["geometry"]]
        .to_crs(satellite_ds.odc.crs)
        .dropna()
    )
    ranks_gdf.columns = ranks_gdf.columns.str.strip("rank_")

    # Loop through each model, interpolating points into low-res tide grid
    out_list = []
    ...
    ranks_ds = xr.concat(out_list, dim="tide_model")

    # Mask out all but the top N models,
    tide_lowres_masked = tide_lowres.where((ranks_ds > (len(models) - top_n)))

    # Take median/mean of remaining to produce a single ensemble output
    # for each time/x/y
    if reduce_method == "mean":
        tide_lowres_ensemble = tide_lowres_masked.mean(dim="tide_model")
    elif reduce_method == "median":
        tide_lowres_ensemble = tide_lowres_masked.median(dim="tide_model")
